...
const totalPages = computed(() =>
    Math.ceil(myAnnouncements.value.length / perPage)
)
...
async function deletePengumuman(id) {
    ...
    try {
        ...
        myAnnouncements.value = myAnnouncements.value.filter(
            (a) => a.id !== id
        )
...
async function deleteAllAnnouncements() {
    ...
    try {
        await axios.delete('/pengumuman/delete-all')
...
<template>
    <MenuLayout>
        <div class="max-w-5xl mx-auto space-y-14 sm:px-4 sm:py-6">
            ...
            <div class="rounded-xl border border-white/20 dark:border-white/10
               ...
               shadow-xl p-6 sm:p-8">
                ...
                <div class="flex items-center gap-3 mb-8">
                    <div class="sm:p-3 p-2 rounded-xl bg-gradient-to-br
                   from-blue-600 to-indigo-600 text-white shadow">
                    ...
                    <div class="flex flex-col">
                        <h1 class="text-xl sm:text-3xl font-bold text-gray-800 dark:text-white">
                            Create Announcement
                            <span class="hidden sm:inline text-gray-400 font-medium">
                                / Message
                            </span>
                        </h1>
                ...
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input v-model="form.judul" placeholder="Enter the title here..."
                            class="w-full mt-1 rounded-xl px-4 py-3 bg-white/80 dark:bg-[#0F172A] border border-gray-300 dark:border-white/10 text-gray-800 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
                    ...
                    <div>
                        ...
                        <div
                            class="relative rounded-xl w-full overflow-hidden border border-gray-300 dark:border-white/10 bg-white dark:bg-[#0F172A] shadow-sm">

                            <QuillEditor v-model:content="form.pengumuman" placeholder="Type your message here..."
                    ...
                    <div class="flex justify-end gap-3 pt-4">
                     ...
                     dark:bg-transparent dark:hover:bg-gray-800 dark:border
                     ...
                     from-indigo-600 to-purple-600 hover:opacity-90
                     text-white font-semibold flex items-center gap-2">
            ...
            <!-- ANNOUNCEMENT LIST -->
            <div
                class="rounded-xl border border-white/20 dark:border-white/10 bg-white/70 dark:bg-white/5 backdrop-blur-xl shadow-xl p-6 sm:p-8">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-xl font-bold text-gray-800 dark:text-white flex items-center gap-2"> 📢 My Recent
                        Messages </h2> <button @click="deleteAllAnnouncements"
                        class="hidden sm:flex items-center gap-2 px-4 py-2 rounded-lg bg-red-600 hover:bg-red-700 text-white text-sm font-semibold">
                ...
                <ul class="space-y-4 max-h-[420px] overflow-y-auto pr-1">
                    <li v-for="item in paginatedAnnouncements" :key="item.id"
                        class="rounded-2xl p-6 bg-white/80 dark:bg-[#0F172A] border border-gray-200 dark:border-white/10 shadow hover:shadow-xl transition">
                        <div class="flex justify-between gap-4">
                            <div> <span
                                    class="inline-block mb-2 px-3 py-1 text-xs font-semibold rounded-full bg-indigo-100 text-indigo-700">
                                    Sent to: {{ displayPenerima(item) }} </span>
                                <h3 class="font-bold text-lg text-gray-800 dark:text-white"> {{ item.judul }} </h3>
                                <div class="prose prose-sm dark:prose-invert max-w-none mt-2 announcement-content"
                                    v-html="item.pengumuman" />
                                <p class="text-xs text-gray-400 mt-2"> {{ new Date(item.created_at).toLocaleString() }}
                                </p>
                            </div> <button @click="deletePengumuman(item.id)" class="text-red-500 hover:text-red-700">
                    ...
                    <li v-if="paginatedAnnouncements.length === 0"
                        class="rounded-xl bg-gray-100 dark:bg-white/5 py-10 text-center text-gray-500"> No announcements
                        yet. </li>
                </ul>
                <!-- PAGINATION -->
                <div class="flex justify-center gap-3 mt-6"> <button @click="prevPage" :disabled="currentPage === 1"
                        class="p-2 rounded-lg bg-gray-200 dark:bg-white/10 hover:bg-gray-300 disabled:opacity-50">
                        <ChevronLeftIcon class="w-5 h-5" />
                    </button> <span class="px-4 py-2 rounded-lg bg-gray-100 dark:bg-white/10"> {{ currentPage }} / {{
                        totalPages }} </span> <button @click="nextPage" :disabled="currentPage === totalPages"
                        class="p-2 rounded-lg bg-gray-200 dark:bg-white/10 hover:bg-gray-300 disabled:opacity-50">
